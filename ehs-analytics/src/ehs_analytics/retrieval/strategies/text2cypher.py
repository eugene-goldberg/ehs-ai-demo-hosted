    ...
    async def initialize(self) -> None:
        ...
        with log_context(component="text2cypher_retriever", operation="initialize"):
            ...
            try:
                ...
                schema_info = {
                    "node_props": len(self.graph.schema.get("node_props", {})),
                    "rel_props": len(self.graph.schema.get("rel_props", {})),
                    "relationships": len(self.graph.schema.get("relationships", []))
                }
                ...
                self.cypher_chain = GraphCypherQAChain.from_llm(
                    ...
                    graph=self.graph,
                    verbose=True,
                    cypher_prompt=self._build_ehs_cypher_prompt(),
                ...
                logger.info(
                    ...
                    schema_nodes=schema_info["node_props"],
                    schema_relationships=schema_info["relationships"]
                )
...
6. For consumption queries, focus on UtilityBill nodes and their relationships
...
- Consumption over time: Match UtilityBill nodes with date ranges
...
Cypher: MATCH (f:Facility)-[:RECORDED_AT]-(u:UtilityBill) 
        WHERE u.utility_type = 'water' AND u.billing_period >= date('2024-07-01') 
        RETURN f.name, SUM(u.amount) as total_consumption
