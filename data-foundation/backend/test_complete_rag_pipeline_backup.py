#!/usr/bin/env python3
"""
Comprehensive RAG Pipeline Integration Test Suite
...
class RAGPipelineComprehensiveTester:
    ...
    def __init__(self):
        ...
        self.log_file = f"/tmp/rag_pipeline_comprehensive_test_{timestamp}.log"
    ...
    def test_intent_classification(self) -> List[TestResult]:
        ...
        for intent_type in self.intent_types:
            ...
            for i, query in enumerate(queries):
                ...
                try:
                    # Classify intent
                    classification_result = self.intent_classifier.classify_intent(query)
                    duration = time.time() - start_time
    ...
    def test_context_retrieval(self) -> List[TestResult]:
        ...
        for intent_type in self.intent_types:
            for site in self.valid_sites:
                ...
                start_time = time.time()
                try:
                    # Get context for intent and site
                    context_data = self.context_retriever.get_context(
                        intent=intent_type,
                        site=site,
    ...
    def test_error_handling(self) -> List[TestResult]:
        ...
        for test_case in error_test_cases:
            ...
            try:
                ...
                classification_result = self.intent_classifier.classify_intent(test_case["query"])
    ...
    def run_comprehensive_test_suite(self) -> Dict[str, Any]:
        ...
        logger.info("=" * 60)
        logger.info("STARTING COMPREHENSIVE RAG PIPELINE TEST SUITE")
        logger.info("=" * 60)
...
def main():
    ...
    # Save report to file
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_file = f"/tmp/rag_pipeline_comprehensive_test_report_{timestamp}.json"
