...
class EndpointVerifier:
    ...
    def test_environmental_endpoints(self):
        ...
        expected_fields = ["id", "content", "source_file", "chunk_index"]
        ...
        for category in categories:
            for data_type in types:
                ...
                if result["status"] == "PASSED":
                    ...
                    location_result["endpoint"] = f"{endpoint}?location=california"
    ...
    def test_llm_assessment(self):
        ...
        test_cases = [
            {
                "params": {
                    "question": "What are the main electricity consumption risks for data centers?",
            ...
            {
                "params": {
                    "question": "How can we reduce water waste in manufacturing?",
                    "category": "water", 
                    "location": "california"
                },
            ...
            {
                "params": {
                    "question": "What are best practices for waste management?",
                    "category": "waste"
                },
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(data, dict) and ("answer" in data or "assessment" in data or "response" in data):
                        answer_text = data.get("answer", "") or data.get("assessment", "") or data.get("response", "")
                        result["response_length"] = len(answer_text)
                        
                        if len(answer_text) > 50:  # Reasonable response length
                            result["status"] = "PASSED"
                            self.log(f"✓ {test_case['name']} - SUCCESS ({result['response_length']} chars, {result['response_time_ms']}ms)")
                        else:
                            result["status"] = "FAILED"
                            result["error"] = "Response too short"
                            self.log(f"✗ {test_case['name']} - FAILED: Response too short")
                    else:
                        result["status"] = "FAILED"
