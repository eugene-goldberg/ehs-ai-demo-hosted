...
def get_context_retriever():
    ...
    if not hasattr(get_context_retriever, "_instance"):
        ...
        if neo4j_client:
            get_context_retriever._instance = ContextRetriever(neo4j_client)
...
async def chat(
    ...
    try:
        ...
        if rag_result.get("augmented_prompt") and not rag_result.get("error"):
            ...
            # For now, use the augmented prompt as the response
            # In the future, this could be sent to an LLM for final generation
            response_text = rag_result["augmented_prompt"]
