...
class UnrejectionRequest(BaseModel):
    """Request model for unrerejecting a document."""
...
async def reject_document(
    ...
    try:
        ...
        rejection_result = await get_rejection_service().reject_document(
            ...
            user_id=current_user["user_id"],  # FIXED: correct parameter name
...
async def unreject_document(
    ...
    try:
        ...
        # Process unreejection - FIXED: correct parameter names
        unreject_result = await get_rejection_service().unreject_document(
            document_id=document_id,
            user_id=current_user['user_id'],  # FIXED: correct parameter name
            notes=request.notes or ""  # FIXED: notes parameter only
        )
        ...
        return {
            ...
            "new_status": "processing"  # FIXED: simplified response
        }
...
async def list_rejected_documents(
    ...
    try:
        ...
        # FIXED: Extract status from filters and pass correctly
        status_filter = filters.get('reason')  # Extract status from filters
